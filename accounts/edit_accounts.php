...
$stmt = mysqli_prepare($connect, "SELECT * FROM comptes WHERE account_id = ?");
...
$clients = mysqli_query($connect, "SELECT client_id, full_name FROM clients");
...
if (isset($_POST['submit'])) {
    ...
    $stmt = mysqli_prepare(
        $connect,
        "UPDATE comptes 
         SET account_number = ?, balance = ?, account_type = ?, client_id = ?
         WHERE account_id = ?"
    );
            ...
            <select name="client_id" required
                ...
                <?php while ($c = mysqli_fetch_assoc($clients)) { ?>
                    <option value="<?php echo $c['client_id']; ?>"
                        <?php if ($c['client_id'] == $account['client_id']) echo 'selected'; ?>>
