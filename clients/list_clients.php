...
$stmt = mysqli_prepare($connect, "SELECT * FROM customers ORDER BY customer_id DESC");
            ...
            <table class="w-full text-left">
                ...
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                    <tr class="border-b border-[#242424] hover:bg-[#222222] transition">
                        <td class="py-3"><?php echo $row['customer_id']; ?></td>
                        <td><?php echo htmlspecialchars($row['full_name']); ?></td>
                        <td><?php echo htmlspecialchars($row['email']); ?></td>
                        <td><?php echo htmlspecialchars($row['phone']); ?></td>
                        <td class="flex gap-3 py-3">
                            <a href="edit_client.php?id=<?php echo (int)$row['customer_id']; ?>"
                               class="text-blue-400 hover:underline">Edit</a>
                            <a href="delete_client.php?id=<?php echo (int)$row['customer_id']; ?>"
